...
const SelectCategorias = ({categoria, setCategoria}) => {
    ...
    const handleClick = (e) => {
        setCategoria(e.currentTarget.dataset.value);
    }
            ...
            {mostrarSelect && 
                <Opciones>
                    {categorias.map((categoria) => {
                        return <Opcion key={categoria.id} onClick={handleClick} data-value={categoria.id}><IconoCategoria id={categoria.id}/>{categoria.texto}</Opcion>
                    })}
                </Opciones>
            }
        </ContenedorSelect>
